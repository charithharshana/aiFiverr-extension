...
class StorageManager {
  ...
  getDefaultSettings() {
    return {
      ...
      googleSearchGrounding: {
        ...
        defaultEnabled: false // Default state for floating icon
      },
      urlContextExtraction: {
        enabled: false, // Default disabled
        defaultEnabled: false, // Default state for floating icon
        autoExtract: true // Automatically extract URL content when URLs are detected
      },
      keyboardShortcuts: {
        aiAssistant: {
          ctrl: true,
  ...
  async cleanupOldSessions() {
    try {
      ...
      if (!this.isExtensionContextValid()) {
        // Return silently instead of logging warning to reduce console spam
        return 0;
      }
      ...
      if (!settings || !sessions || typeof sessions !== 'object') {
        // Return silently for invalid data to avoid spam
        return 0;
      }
